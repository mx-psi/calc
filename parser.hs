...
data Expression = Var String | Val Double | Op String [Expression] deriving (Eq, Show)
...
parse [] ys
  ...
  | '_' `elem` ys = let (op, val) = span (/= '_') ys in Op op [parse [] val]
  | otherwise = Var ys
...
-- Ops parsing
parse (op : ops) ys
  | op `isInfixOf` ys = Op op (map (parse ops) (splitOn op ys))
