...
parse [] ys
  | '_' `elem` ys
  = let (op, val) = span (/= '_') ys in Node op [parse [] val]
  | otherwise
  = return ys
...
parse opl ys | '(' `elem` ys =
  let
    bs    = map (slice ys) (coordsList ys)   -- String content of brackets
    expBs = map (parse opl . tail . init) bs
  in substituteList (zip expAux expBs) $ parse opl $ replaceList
    (zip bs varsList)
    ys -- Parsed contents
  where expAux = map return varsList
